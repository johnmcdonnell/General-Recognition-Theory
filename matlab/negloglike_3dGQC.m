function negloglike = negloglike_3dGQC(params,data,z_limit,catflag)
%negloglike = negloglike_3dGQC(params,data,z_limit,switch)
...
%                  added catflag to switch A and B response categories
...
pnoise = params(1);
cnoise = params(2);
A = [   params(3) .5*params(6) .5*params(7)
     .5*params(6)    params(4) .5*params(8)
     .5*params(7) .5*params(8)    params(5)];
b = params(9:11)';
c = params(12);

% Compute mean of h(x) for each x (as shown in Eqn. 11, Ashby p.462).
temp1 = trace(A*pnoise);
xAxs = params(3)*data(:,2).^2 + params(4)*data(:,3).^2 + params(5)*data(:,4).^2 ...
       + ( params(6)*prod(data(:,2:3)') + params(7)*prod([data(:,2),data(:,4)]') + params(8)*prod(data(:,3:4)') )';
meanhxs = temp1 + xAxs + data(:,2:4)*b + c;
...
bvec = [b(1)*ones(1,ndatapts);b(2)*ones(1,ndatapts);b(3)*ones(1,ndatapts)];
temp2 = bvec+2*A*data(:,2:4)';
