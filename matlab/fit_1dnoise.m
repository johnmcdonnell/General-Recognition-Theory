function [out_params, neglikelihood] = fit_1dnoise(in_params, data, z_limit)
%[out_params, neglikelihood] = fit_1dnoise(in_params, data, z_limit)
...
start_params = [in_params(1) in_params(3)];

% Set contraints on parameters
vlb = [ .001 in_params(2) start_params(3)];
vub = [ 500  in_params(2) start_params(3)];
...
% Search!
final_params = fmincon('negloglike_1dGLC',start_params,[],[],[],[],vlb,vub,[],options,data,z_limit);

% Report results
out_params = final_params;
neglikelihood = negloglike_1dGLC(final_params,data,z_limit);
